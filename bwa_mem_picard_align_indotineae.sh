...
## Versions used:
## BWA aligner: 0.7.17
## SAMTools: samtools_1.16.1
## Picard: 2.27.4
## Java: jdk-8u66
## GATK: 4.2
#####################################################################

mkdir -p -v /rds/general/project/trichophyton/ephemeral/$3

reference_dir=/rds/general/project/trichophyton/live/reference/indotineae
reference=$reference_dir/GCA_023065905.1_ASM2306590v1_genomic.fa
output=/rds/general/project/trichophyton/ephemeral/$3
results_dir=/rds/general/project/trichophyton/live/results/mapping_to_indotineae/$3
tmp=/rds/general/project/trichophyton/ephemeral/
rgid=A00478-$3-233
rgpu=233
...
samtools view -bS $output/$3.sam > $output/$3.bam
samtools sort $output/$3.bam -o $output/$3.sorted.bam
samtools index $output/$3.sorted.bam

#Activate picard environment

source activate picard_env
...
#Call variants
gatk HaplotypeCaller --tmp-dir $tmp -R $reference -I $output/$3.sorted.marked.bam -O $output/$3.raw_variants.vcf --pcr-indel-model NONE -ploidy 1 -stand-call-conf 30 -mbq 20 -A QualByDepth -XL $reference_dir/GCA_023065905.1_ASM2306590v1_genomic.fa.repeat.intervals
...
#Call variants (again)
gatk HaplotypeCaller --tmp-dir $tmp -R $reference -I $output/$3.post_recal_reads.bam -O $output/$3.raw_variants_recal.vcf -ERC GVCF --pcr-indel-model NONE -ploidy 1 -stand-call-conf 30 -mbq 20 -A QualByDepth -XL $reference_dir/GCA_023065905.1_ASM2306590v1_genomic.fa.repeat.intervals  
